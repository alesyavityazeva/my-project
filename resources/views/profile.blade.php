    ...
    <script>
        function confirmCancellation(form) {
            if (confirm('Вы уверены, что хотите отменить запись?')) {
                ...
                fetch('{{ route("booking.cancel") }}', { method: 'POST', body: formData })
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (editForm) {
                editForm.addEventListener('submit', function(e) {
                    e.preventDefault();
                    const formData = new FormData(this);
                    fetch('{{ route("booking.update") }}', { method: 'POST', body: formData })
                    .then(response => response.json())
                    .then(data => {
    ...
    <div class="container">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; padding: 0 10px;">
            ...
            <div style="display: flex; gap: 15px;">
                @if($user_role == 2)
                    <a href="{{ route('admin.index') }}" class="nav-btn" style="margin: 0; padding: 10px 20px; font-size: 0.95em;">Панель админа</a>
                    <a href="{{ route('home') }}" class="nav-btn" style="margin: 0; padding: 10px 20px; font-size: 0.95em;">Клиентская страница</a>
                @elseif($user_role == 1)
                    <a href="{{ route('master.index') }}" class="nav-btn" style="margin: 0; padding: 10px 20px; font-size: 0.95em;">Панель мастера</a>
                    <a href="{{ route('home') }}" class="nav-btn" style="margin: 0; padding: 10px 20px; font-size: 0.95em;">Клиентская страница</a>
                @else
                    <a href="{{ route('home') }}" class="nav-btn" style="margin: 0; padding: 10px 20px; font-size: 0.95em;">На главную</a>
                @endif
                <button onclick="confirmLogout()" class="logout-btn" style="margin: 0; padding: 10px 20px; font-size: 0.95em;">Выйти</button>
            </div>
        </div>

        @if($user_role == 1 && $stats)
        <div class="profile-container statistics-container">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-icon">&#128181;</div>
                    <div class="stat-value">{{ number_format($salary ?? 0, 2) }} &#8381;</div>
        ...
        <div class="profile-container">
            ...
            <form action="{{ route('profile.update') }}" method="post" class="profile-info">
                ...
                <input type="text" id="lastname" name="lastname" value="{{ $user_info->lastname ?? '' }}" required pattern="[А-Яа-яЁё\s\-]+">
                <label for="name">Имя:</label>
                <input type="text" id="name" name="name" value="{{ $user_info->name ?? '' }}" required pattern="[А-Яа-яЁё\s\-]+">
                <label for="firstname">Отчество:</label>
                <input type="text" id="firstname" name="firstname" value="{{ $user_info->firstname ?? '' }}" pattern="[А-Яа-яЁё\s\-]*">
                <label for="phone">Номер телефона:</label>
                <input type="tel" id="phone" name="nomber_tel" value="{{ $user_info->nomber_tel ?? '' }}" required>
        ...
        <div class="profile-container">
            ...
            @if($active_bookings->count() > 0)
                <div class="total-price">
                    ...
                    <span class="price-highlight">{{ number_format($total_price, 2) }} &#8381;</span>
                </div>
                <ul class="bookings-list">
                @foreach($active_bookings as $booking)
                    <li class="booking-item">
                        <strong>Услуга:</strong> {{ $booking->service_name }}<br>
                        <strong>Цена:</strong> <span class="price-text">{{ number_format($booking->service_price, 2) }} &#8381;</span><br>
                        <strong>Мастер:</strong> {{ $booking->master_name }}<br>
                        <strong>Дата и время:</strong> {{ $booking->date_time }}<br>
                        <div class="booking-actions">
                            <button type="button" class="edit-btn" onclick="openEditModal({{ $booking->id_zapis }}, 0, 0, '{{ $booking->date_time }}')">Редактировать</button>
                            <form action="{{ route('booking.cancel') }}" method="post" style="display: inline;" onsubmit="event.preventDefault(); confirmCancellation(this);">
                                @csrf
                                <input type="hidden" name="booking_id" value="{{ $booking->id_zapis }}">
